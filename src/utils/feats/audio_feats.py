...
import src.vendor.loupe_keras as lpk
...
def main(data_dir:str, out_dir:str, ssds_threshold:int, cluster_size:int):
    ...
    for item_dir in sorted(glob.glob(data_dir + r'/*/*'),
        ...
        for item_path in glob.glob(item_dir + r'/*_out.wav'.format(item_dir)):
            ...
            vlad_feat = lpk.NetVLAD(feature_size=feature_size, max_samples=max_samples, cluster_size=cluster_size, output_dim=cluster_size * 16)(tf.convert_to_tensor(log_mel_spec)).numpy()
    ...
    np.savez(os.path.join(out_dir, 'audio_feats.npz'), feats=feats, targets=targets, dep_idxs=dep_idxs, non_idxs=non_idxs)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-o', '--out_dir', type=str, help='Directory to save audio_feats.npz file', default=None)
    parser.add_argument('-t', '--ssds_threshold', type=int, help='Threshold for standard SDS score', default=53)
    parser.add_argument('-c', '--cluster_size', type=int, help='Cluster size for NetVLAD', default=16)
