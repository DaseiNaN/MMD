...
def main(data_dir: str, out_dir: str, ssds_threshold: int):
    ...
    for item_dir in sorted(
        ...
        for item_path in glob.glob(item_dir + r"/*.txt"):
    ...
    np.savez(
        os.path.join(out_dir, "text_feats.npz"),
        feats=feats,
        targets=targets,
        dep_idxs=dep_idxs,
        non_idxs=non_idxs,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-o", "--out_dir", type=str, help="Directory to save text_feats.npz file", default=None
    )
    parser.add_argument(
